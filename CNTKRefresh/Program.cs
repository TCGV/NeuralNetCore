...
namespace CNTKRefresh
{
    class Program
    {
        ...
        bool loadData(string filename)
        {
            ...
            characters = corpus.Where(c => c != ' ').Distinct().ToList();
            characters.Sort();
        ...
        {
            SetDevice();
            if (!loadData( @"..\CNTKRefresh\Data\tinyshakespeare.txt")) return;
            if (!loadModel( @"..\CNTKRefresh\Model\shakespeare_epoch41.dnn")) return;
